...
const QuantitySelect:React.FC<QuantitySelectProps> = ({product, className}: QuantitySelectProps) => {
  const dispatch = useAppDispatch()
  const currCartItem = useAppSelector(cartItems).find(item => item.id === product.id)
  ...
  const [btnText, setBtnText] = useState(qtyInCart !== '' ?  BTNTEXT.UPDATE : BTNTEXT.ADD)
  ...
  useEffect(() => {
    ...
    setBtnText(() => {
      return qtyInCart === '0' ? BTNTEXT.REMOVE : qtyInCart !== '' ? BTNTEXT.UPDATE : BTNTEXT.ADD
    }) 
  ...
  const handleClick = () => {
    ...
    const msg = qtyInCart  ? `Updated ${product.name} quantity to ${selectedValue}` : `Added ${product.name} from your cart`
    toast.success(msg)
  }
        ...
        <Select.Root size="1" value={qtyInCart ? selectedValue : selectedValue.replace('0','')} onValueChange={handleChange}>
          ...
          <Select.Content className="text-sm">
              <Select.Group>
                  ...
                  {!!qtyInCart && <Select.Item value="0" key={0}>0</Select.Item>}
                  {qtyArray.map((qty) => <Select.Item key={qty} value={qty.toString()}>{qty}</Select.Item>)}
              </Select.Group>
          </Select.Content>
        </Select.Root>
      </div>
